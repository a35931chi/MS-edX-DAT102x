...
def data_wrangle():
    ...
    Xtrain = df[~np.isnan(df['poverty_rate'])]
    Xtest = df[np.isnan(df['poverty_rate'])].drop('poverty_rate', axis = 1)



    Xtrain, Xval, ytrain, yval = train_test_split(Xtrain.drop('poverty_rate', axis = 1),
                                                  Xtrain['poverty_rate'], test_size=0.3, random_state=42)
    ...
    return Xtrain_scaled, ytrain, Xval_scaled, yval, Xtest_scaled
...
if __name__ == '__main__':
    lam = 0.15
    Xtrain_scaled, ytrain, Xval_scaled, yval, Xtest_scaled = data_wrangle()
    
    ENetR_model = ENetR_GSCV(Xtrain_scaled, Xval_scaled, ytrain, yval)
    LassoR_model = Lasso_GSCV(Xtrain_scaled, Xval_scaled, ytrain, yval)
    XGBR_model = XGBR_GSCV(Xtrain_scaled, Xval_scaled, ytrain, yval)

    prediction = invboxcox(ENetR_model.predict(Xtest_scaled), lam)
    output = pd.DataFrame()
    output['row_id'] = pd.read_csv('test_values.csv')['row_id']
    output['poverty_rate'] = prediction
